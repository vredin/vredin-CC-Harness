...
> _Vendored from anthropics/claude-plugins-official @ 4bf0858 (2026-05-19). Source: https://github.com/anthropics/claude-plugins-official/tree/4bf0858/plugins/session-report_
...
## Local Caveat (T-OSF-001)

If `--since 24h` returns 0 sessions but you've been using Claude Code recently — sessions may be stored elsewhere (e.g. by the `context-mode` plugin, or in a Desktop build location not at `~/.claude/projects/`). Try `--since all` first to confirm the script works on historical data. If only old data appears: check `~/.claude/plugins/marketplaces/*/hooks/` for plugins intercepting session storage.
